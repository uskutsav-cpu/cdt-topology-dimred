...
from .summary_storage import CompactRootCube,reduce_roots
...
def _summarize_curves(cube,pop,floors,cfg,key):
    C,S,R,L=cube.shape
    if isinstance(cube,CompactRootCube):
        p=cube.means;root_se=cube.root_error;half=cube.half_ds.mean(axis=(0,1))
    else:
        p=cube.mean(axis=2);root_se=root_jackknife(cube,pop)
...
def analyze_study(root,cfg,study):
    ...
    source_files=['src/cdt_baseline/analysis.py','src/cdt_baseline/summary_storage.py','src/cdt_baseline/statistics.py','src/cdt_baseline/design.py','src/cdt_baseline/io.py','src/cdt_baseline/chain.py']
    ...
    with lock(out.parent/(key+'.lock')):
        ...
        for (volume,k0),entries in sorted(groups.items()):
            ...
            for row in rows:
                ...
                with np.load(mpath.parent/'returns.npz',allow_pickle=False) as z:
                    for curve in m['curves']:
                        label=curve['key'];data.setdefault(label,{})[c,s]=reduce_roots(z[label],curve['population']);curve_metadata.setdefault(label,{})[c,s]=curve
            ...
            for label,values in sorted(data.items()):
                ...
                cube=CompactRootCube.from_summaries(values,C,S)
