...
rows=[];inputs={};curves=[]
for job,target,stride in [('4a40078dbf0e6b9a5f5c',10000,4),('dc0bd0d263f25696be2d',30000,8)]:
...
with (T/'finite_size_pilot.csv').open('w') as f:
    w=csv.DictWriter(f,fieldnames=list(rows[0]));w.writeheader();w.writerows(rows)
(T/'finite_size_pilot.json').write_text(json.dumps({'status':'exploratory; two volumes do not establish asymptotic scaling','rows':rows,'input_sha256':inputs},indent=2)+'\n')
...
for ext in ['pdf','png']:fig.savefig(ROOT/'results/figures'/f'finite_size_pilot.{ext}',dpi=160)
